...
class ListenerConfig(BaseModel):
    service_name: str = Field("mindie", description="服务名称")
    port: int = Field(8082, description="守护进程服务端口")
    listen_port: int = Field(8081, description="mindie监听端口")
    work_dir: str = Field(os.getcwd(), description="工作目录 (默认为当前文件夹)")
    sh_file: str = Field('mindie_start.sh', description="mindie启动脚本")
    ...
    startup_wait_time: float = Field(60.0, description="mindie启动等待时间")
...
def check_mindie_health(port: int = 8081):
    """检查mindie服务是否正常"""
    try:
...
def restart_mindie_service(listener_config: ListenerConfig):
    """启动mindie服务"""
    logger.info("*" * 50)
    ...
    try:
        ...
        logger.info(
            f"Service {listener_config.service_name} is restarting...,please wait at least {listener_config.startup_wait_time} seconds!!!!!!")
...
def periodic_health_check(listener_config: ListenerConfig):
    ...
    while True:
        try:
            is_health = check_mindie_health(port=listener_config.listen_port)  # 健康检查
            ...
            if (not is_health) and (not gpu_occupied):  # 如果健康检查失败且GPU未被占用，则重启服务
                ...
                restart_mindie_service(listener_config=listener_config)
            else:
                if not is_health:  # 健康检查失败，但是有显存占用，则可能是还在启动过程中，或者有显存占用
                    logger.info(
                        f"Service {listener_config.service_name} is not healthy, but GPU is occupied, not restart")
                    logger.info(
                        f"Please wait for the GPU to be released,or for the next health check in {listener_config.check_interval} seconds")
                else:  # 健康检查成功
...
async def health_check():
    """健康检查接口"""
    mindie_health = check_mindie_health(port=listener_config.listen_port)
    gpu_occupied = if_npus_occupied(memory_threshold=listener_config.memory_threshold,
                                    num_threshold=listener_config.num_threshold
                                    )  # GPU占用
    return {"code": 200,
            "data": {"mindie_health": mindie_health,
...
if __name__ == "__main__":
    ...
    log_path = os.path.join(listener_config.log_dir, f"mindie_service_{timestamp}.log")

    SERVICE_COMMAND = {
        "mindie": f"nohup bash {os.path.join(listener_config.work_dir, listener_config.sh_file)} >> {log_path} 2>&1 &"
    }
    logger.info(f"*" * 100)
    logger.info(f"*" * 50)
    logger.info(f"Starting mindie listener with configuration:")
    ...
    # # 启动服务
    # restart_mindie_service(listener_config)
