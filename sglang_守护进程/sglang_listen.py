...
class ListenerConfig(BaseModel):
    service_name: str = Field("sglang", description="服务名称")
    port: int = Field(8082, description="守护进程服务端口")
    listen_port: int = Field(8081, description="sglang监听端口")
    work_dir: str = Field(os.getcwd(), description="工作目录 (默认为当前文件夹)")
    sh_file: str = Field('sglang_start.sh', description="sglang启动脚本")
    ...
    startup_wait_time: float = Field(60.0, description="sglang启动等待时间")
...
def check_sglang_health(port: int = 8081):
    """检查sglang服务是否正常"""
    try:
...
def restart_sglang_service(listener_config: ListenerConfig):
    """启动sglang服务"""
    logger.info("*" * 50)
...
def periodic_health_check(listener_config: ListenerConfig):
    ...
    while True:
        try:
            is_health = check_sglang_health(port=listener_config.listen_port)  # 健康检查
            gpu_occupied = if_gpus_occupied(memory_threshold=listener_config.memory_threshold,
            ...
            if (not is_health) and (not gpu_occupied):  # 如果健康检查失败且GPU未被占用，则重启服务
                ...
                restart_sglang_service(listener_config=listener_config)
            else:
...
async def health_check():
    """健康检查接口"""
    sglang_health = check_sglang_health(port=listener_config.listen_port)
    gpu_occupied = if_gpus_occupied(memory_threshold=listener_config.memory_threshold,
                                    num_threshold=listener_config.num_threshold
                                    )  # GPU占用
    return {"code": 200,
            "data": {"sglang_health": sglang_health,
...
if __name__ == "__main__":
    ...
    log_path = os.path.join(listener_config.log_dir, f"sglang_service_{timestamp}.log")

    SERVICE_COMMAND = {
        "sglang": f"nohup bash {os.path.join(listener_config.work_dir, listener_config.sh_file)} >> {log_path} 2>&1 &"
    }
    logger.info(f"*" * 100)
    logger.info(f"*" * 50)
    logger.info(f"Starting sglang listener with configuration:")
    ...
    # # 启动服务
    # restart_sglang_service(listener_config)
